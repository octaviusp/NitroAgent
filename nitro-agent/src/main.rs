...
use teloxide::types::{AllowedUpdate, MaybeInaccessibleMessage, MessageEntityKind, UpdateKind};
...
async fn main() {
    ...
    for (name, config) in agents {
        ...
        if let Err(e) = tg.set_my_commands(menu::bot_commands()).await {
            warn!(agent = %name, "Failed to set bot commands: {e}");
        } else {
            info!(agent = %name, "Bot commands registered");
        }
                ...
                for update in updates {
                    ...
                    if let Some(cmd_text) = menu::match_keyboard_button(&text) {
                        ...
                        if let Err(e) = worker.enqueue(task) {
                            error!(agent = %name, "Failed to enqueue keyboard task: {e}");
                        }
                        continue;
                    }
