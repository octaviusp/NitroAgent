...
_DEFAULT_CACHE_DIR: Final[Path] = _PROJECT_ROOT / ".cache" / "moonshine_voice"
...
class SST:
    ...
    def __new__(cls, *args: object, **kwargs: object) -> SST:
    ...
    def __init__(
        ...
        resolved_cache_dir = (
            Path(cache_dir).expanduser().resolve() if cache_dir else _DEFAULT_CACHE_DIR.resolve()
        )
        ...
        if getattr(self, "_initialized", False):
            if (
                ...
                raise RuntimeError(
                    "SST singleton already initialized with different settings. "
                    "Create it once with your desired language/arch/cache_dir."
                )
            return
        ...
        self.model_path, self.model_arch = _resolve_model(self.language, self.arch)

        self._transcriber = Transcriber(model_path=self.model_path, model_arch=self.model_arch)
    ...
    def transcript(self, audio: AudioInput) -> Text:
        ...
        `audio` accepts:
        - `bytes` / `bytearray` / `memoryview` (recommended for Telegram bot payloads)
        ...
        with _TempFiles() as temp_files:
            ...
            wav_path = temp_files.normalize_to_wav(source_path)
...
class _TempFiles:
    ...
    def _new_path(self, suffix: str) -> Path:
        with tempfile.NamedTemporaryFile(mode="wb", suffix=suffix, delete=False) as file_obj:
    ...
    def materialize(self, audio: AudioInput) -> Path:
        ...
        raise TypeError("audio must be bytes, bytearray, memoryview, str, or Path")

    def normalize_to_wav(self, source_path: Path) -> Path:
        ...
        if result.returncode != 0:
            ...
            raise RuntimeError(f"ffmpeg audio normalization failed:\n{stderr_tail}")
        return output_path
...
def get_sst(
    ...
    """Factory helper that always returns the same singleton instance."""
    return SST(language=language, arch=arch, cache_dir=cache_dir)
...
def _parse_arch(value: str | ModelArch) -> ModelArch:
    ...
    if lowered.isdigit():
        try:
            ...
            raise ValueError(f"Invalid numeric model arch '{value}'.") from exc

    valid = ", ".join(sorted(_ARCH_BY_NAME.keys()))
    raise ValueError(f"Invalid model arch '{value}'. Valid values: {valid}, or numeric enum.")
...
def _resolve_model(language: str, arch: ModelArch) -> tuple[str, ModelArch]:
...
def _main() -> int:
    parser = argparse.ArgumentParser(description="Self-contained singleton STT utility")
    ...
    stt = get_sst(language=args.language, arch=args.arch, cache_dir=args.cache_dir)
